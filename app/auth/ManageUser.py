...
async def createUser(unique_name,group_name,temp_session):
    ...
    raise HTTPException(status_code=200, detail="User created successfully "+str(res))

async def deleteUser(unique_name,temp_session):
    try:
        iam_client = temp_session.client('iam')
        dynamodb_client = temp_session.resource('dynamodb',region_name=os.getenv('AWS_REGION'))
        table = dynamodb_client.Table('users')
        res= table.get_item(Key={'username': unique_name})
        if not res.get('Item'):
    ...
    try:
        res=iam_client.get_login_profile(UserName=unique_name)
        if res:
            iam_client.delete_login_profile(UserName=unique_name)
    except iam_client.exceptions.ClientError as e:
        pass#iam_client.delete_user(UserName=unique_name)
